...
const Forms: React.FC<{ inSideModal: boolean; location: string }> = ({
  ...
  // Define the site key and callback directly inside the component
  const siteKey = "6LccDc8qAAAAAO4Cbpf2tk9_5SEH148MHJbhZ2-a"; 
  ...
  const handleVerify = (token) => {
    ...
    fetch("https://api.vetic.in/lead/verify", {
      method: "POST",
      headers: {
      ...
      <div className={inSideModal ? "px-2" : "w-full h-full"}>
        {!showThankYou ? (
          <form onSubmit={handleSubmit} className=" ">
            ...
            <div>
              <div id="recaptcha-container" className="sm:mb-6 mb-4"></div>
              {captchaToken && <p className="text-green-600">CAPTCHA Verified!</p>}
            </div>
            {/* {captchaToken && <p>CAPTCHA Verified! Token: {captchaToken}</p>} */}
            {/* <ReCAPTCHA
                      sitekey="6LccDc8qAAAAAO4Cbpf2tk9_5SEH148MHJbhZ2-a" // Replace with your actual Site Key
                      onChange={handleCaptchaChange}
                    /> */}

            <button
              type="submit"
              disabled={!isFormValid || spin}
              className={`${
                isFormValid
                  ? spin
                    ? "bg-[#152239] cursor-not-allowed flex align-center justify-center"
                    : "bg-[#152239] hover:bg-[#152239]"
            ...
            >
              ...
              {spin ? (
                <img
                  ...
                  alt="lead"
                  className="w-6 h-6 bg-[#152239] rounded-2xl"
                />
              ) : (
                "Book Appointment"
              )}
            ...
            <div className="flex justify-center items-center font-bricolage font-light text-xs mt-2">
              ...
              Ratings | 70,000+ Happy Pet Parents
            </div>
          </form>
        ) : (
